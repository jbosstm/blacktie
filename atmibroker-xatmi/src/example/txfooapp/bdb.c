...
int opendb(DB **dbp, const char *backing_file, const char * dbname)
{
	...
	userlogc((char*) "opendb %s %s", backing_file, (dbname ? dbname : "no name"));
...
userlogc((char*) "doSelect key=%s", kv);
	*rcnt = 0;
    ...
    while ((ret = dbcp->get(dbcp, &key, &data, DB_NEXT)) == 0) {
userlogc((char*) "record: %s=%s", (char *)key.data, (char *)data.data);
		*rcnt += 1;
		if (vv != NULL && kv != NULL && strcmp(kv, (char *)key.data) == 0) {
			strcpy(*vv, (char *)data.data);
			*(*vv + data.size) = 0;
...
userlogc((char*) "doSelect %d records", *rcnt);
    if (ret != DB_NOTFOUND)
        dbp->err(dbp, ret, "DBcursor->get");
...
static int doWork(DB *dbp, char op, char *arg, test_req_t *resp)
{
    ...
    logit(0, "doWork op=%c arg=%s", op, arg);
