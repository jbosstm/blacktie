...
{
	if (xid) {
		LOG4CXX_TRACE(xaResourceLogger, 
...
XAResourceManager::~XAResourceManager() {
	...
	LOG4CXX_TRACE(xaResourceLogger, (char *) "xa_close: " << rv);

	if (rv != XA_OK)
		LOG4CXX_WARN(xaResourceLogger, (char *) " close RM " << name_ << " failed: " << rv);
...
//			LOG4CXX_TRACE(xaResourceLogger,  (char *) "created poa");
		...
		try {
			...
			LOG4CXX_WARN(xaResourceLogger, (char *) "Duplicate RM POA - name = " << name);
			RMException ex("Duplicate RM POA", EINVAL);
			throw ex;
		}

	} catch (PortableServer::POA::InvalidPolicy &) {
		LOG4CXX_WARN(xaResourceLogger, (char *) "Invalid RM POA policy");
...
int XAResourceManager::createServant(XID * xid)
{
	...
	try {
		...
		LOG4CXX_TRACE(xaResourceLogger, (char*) "narrowing resource");

		CosTransactions::Resource_var v = CosTransactions::Resource::_narrow(ref);

		// enlist it with the transaction
		LOG4CXX_TRACE(xaResourceLogger, (char*) "enlisting resource");
		...
		if (CORBA::is_nil(rc)) {
			LOG4CXX_TRACE(xaResourceLogger, (char*) "createServant: nill RecoveryCoordinator ");
		} else {
			XID * cp = (XID *) malloc(sizeof(XID));

			if (cp == 0) {
				LOG4CXX_ERROR(xaResourceLogger, (char *) "out of memory");
				return XAER_RMFAIL;
			}
		...
		LOG4CXX_ERROR(xaResourceLogger, (char*) "createServant: poa inactive");
	} catch (CosTransactions::Inactive&) {
		LOG4CXX_TRACE(xaResourceLogger, (char*) "createServant: tx inactive (too late for registration)");
...
{
	LOG4CXX_TRACE(xaResourceLogger, (char*) "notifyError reason:" << xa_error);
...
void XAResourceManager::setComplete(XID * xid)
{
	...
	LOG4CXX_TRACE(xaResourceLogger, (char*) "RM removing branch: "
		<< xid->formatID << ':'
		<< xid->gtrid_length << ':'
		<< xid->bqual_length << ':'
		<< xid->data);
	...
	LOG4CXX_TRACE(xaResourceLogger, (char*) "... unknown branch");
}
