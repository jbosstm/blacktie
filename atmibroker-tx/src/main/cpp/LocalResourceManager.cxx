...
LoggerPtr loggerLocalResourceManager(Logger::getLogger("LocalResourceManager"));

LocalResourceManager::LocalResourceManager(ResourceManagerDataStruct& aResourceManagerDataStruct) {
	LOG4CXX_LOGLS(loggerLocalResourceManager, Level::getInfo(), (char*) "constructor");
	...
	if (CORBA::is_nil(tmp_ref)) {
		LOG4CXX_LOGLS(loggerLocalResourceManager, Level::getError(), (char*) "Could not find AtmiBrokerXA_ResourceManagerFactory");
	} else {
		LOG4CXX_LOGLS(loggerLocalResourceManager, Level::getInfo(), (char*) "tmp_ref is " << (void *) tmp_ref);
		m_resourceManagerFactory = AtmiBrokerXA::ResourceManagerFactory::_narrow(tmp_ref);
		if (CORBA::is_nil(m_resourceManagerFactory)) {
			LOG4CXX_LOGLS(loggerLocalResourceManager, Level::getError(), (char*) "Could not narrow reference to interface AtmiBrokerXA::ResourceManagerFactory");
		} else {
			LOG4CXX_LOGLS(loggerLocalResourceManager, Level::getInfo(), (char*) "m_resourceManagerFactory is " << (void *) m_resourceManagerFactory);
			...
			LOG4CXX_LOGLS(loggerLocalResourceManager, Level::getInfo(), (char*) "Remote ResourceManager is " << aResourceManagerIOR);
			tmp_ref = AtmiBrokerOTS::get_instance()->getOrb()->string_to_object(aResourceManagerIOR);
			LOG4CXX_LOGLS(loggerLocalResourceManager, Level::getInfo(), (char*) "Remote ResourceManager is " << (void *) tmp_ref);
			m_resourceManagerDataStruct.resourceManager = XA::ResourceManager::_narrow(tmp_ref);
			LOG4CXX_LOGLS(loggerLocalResourceManager, Level::getInfo(), (char*) "Narrowed Remote ResourceManager is " << (void *) m_resourceManagerDataStruct.resourceManager);
		}
	}
}

LocalResourceManager::~LocalResourceManager() {
	LOG4CXX_LOGLS(loggerLocalResourceManager, Level::getInfo(), (char*) "destructor");
...
CORBA::ULong LocalResourceManager::register_before_completion_callback(XA::BeforeCompletionCallback_ptr bcc) throw(CORBA::SystemException) {
	LOG4CXX_LOGLS(loggerLocalResourceManager, Level::getInfo(), (char*) "register_before_completion_callback");
...
void LocalResourceManager::unregister_before_completion_callback(unsigned int key) throw(CORBA::SystemException) {
	LOG4CXX_LOGLS(loggerLocalResourceManager, Level::getInfo(), (char*) "unregister_before_completion_callback");
...
LocalResourceManager::getResourceManagerDataStruct() {
	LOG4CXX_LOGLS(loggerLocalResourceManager, Level::getInfo(), (char*) "getResourceManagerDataStruct");
