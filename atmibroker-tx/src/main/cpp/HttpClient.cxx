...
static char *read_response(struct mg_request_info* ri, struct mg_connection *conn, size_t *rcnt)
{
	int nread = 0;
	char *buf = (char *) calloc(BUFSIZ, sizeof (char));
	int n = read_bytes(conn, buf, BUFSIZ, &nread);
	char *body = NULL;
//	char *body = (nread > n ? mg_strndup(buf + n, nread - n + 1) : NULL);
	char *b = buf;
	char *scode;

body = (nread > n ? mg_strndup(buf + n, nread - n) : NULL);
	if (rcnt != NULL)
		*rcnt = (size_t) (nread - n + 1);

	LOG4CXX_DEBUG(httpclientlog, "read response: " << n << " and " << nread << " body: " << body);
...
free(buf);
if (clen != NULL && (n = atoi(clen)) > 0) {
	*(body + n) = '\0';
	LOG4CXX_DEBUG(httpclientlog, "clen=" << n << " body=" << body);
//	*(b + n) = '\0';
//	n = strlen(b);
//	body = strdup(b);
}
...
//	printf("connected to TM on %s:%d (%s %s)\n", host, port, method, uri);
	...
	char *content = read_response(ri, conn, rcnt);
...
#if 0
// this works but causes valgrind to report invalid reads
char * HttpClient::send(struct mg_request_info* ri,
	const char* method, const char* uri, const char* mediaType, const char *body, size_t *rcnt) {
	size_t blen = (body == NULL ? 0 : strlen(body));
...
//	printf("connected to TM on %s:%d (%s %s)\n", host, port, method, uri);
	...
	char *content = (nread > n ? mg_strndup(buf + n, nread - n + 1) : NULL);
	char *b = & buf[0];
	char *scode;
