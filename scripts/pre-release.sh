...
do
    ...
    find . -name \*.java -o -name \*.xml -o -name \*.properties -o -name \*.ent -o -name \INSTALL -o -name \README -o -name \*.sh -o -name \*.cxx -o -name \*.c -o -name \*.cpp | grep -v ".svn" | grep -v ".git" | grep -v target | grep -v .idea | xargs sed -i "s/$CURRENT-SNAPSHOT/$CURRENT/g" || fatal
    git commit -am "Updated to $CURRENT" || fatal
    git tag $CURRENT || fatal

    find . -name \*.java -o -name \*.xml -o -name \*.properties -o -name \*.ent -o -name \INSTALL -o -name \README -o -name \*.sh -o -name \*.cxx -o -name \*.c -o -name \*.cpp | grep -v ".svn" | grep -v ".git" | grep -v target | grep -v .idea | xargs sed -i "s/$CURRENT/$NEXT/g" || fatal
