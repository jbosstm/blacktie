...
ant dist -DBT_HOME=$WORKSPACE/trunk/dist/ -DVERSION=blacktie-3.0.0.M3-SNAPSHOT -DMACHINE_ADDR=`hostname` -DJBOSSAS_IP_ADDR=localhost -Dbpa=centos54x64
if [ "$?" != "0" ]; then
	ps -f
	$WORKSPACE/jboss-5.1.0.GA/bin/shutdown.sh -S && cd .
	killall -9 testsuite
...
export DB2_DIR=/opt/ibm/db2/V9.7
