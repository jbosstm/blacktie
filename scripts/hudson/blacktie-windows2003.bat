...
rem BUILD BLACKTIE
cd %WORKSPACE%\trunk
call mvn clean -gs tools\maven\conf\settings.xml 
...
call ant dist -DBT_HOME=%BT_HOME% -DVERSION=blacktie-5.0.0.M1-SNAPSHOT -DJBOSSAS_IP_ADDR=%JBOSSAS_IP_ADDR% -DMACHINE_ADDR=%MACHINE_ADDR%
IF %ERRORLEVEL% NEQ 0 echo "Failing build" & tasklist & call %WORKSPACE%\jboss-5.1.0.GA\bin\shutdown.bat -s %JBOSSAS_IP_ADDR%:1099 -S & echo "Failed build" & exit -1

rem RUN THE SAMPLES
cd %WORKSPACE%
call ant initializeBlackTieQuickstartSecurity
cd %WORKSPACE%\trunk\dist\blacktie-5.0.0.M1-SNAPSHOT
IF %ERRORLEVEL% NEQ 0 echo "Failing build" & tasklist & call %WORKSPACE%\jboss-5.1.0.GA\bin\shutdown.bat -s %JBOSSAS_IP_ADDR%:1099 -S & echo "Failed build" & exit -1
