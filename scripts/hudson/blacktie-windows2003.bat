...
echo        ^<replaceregexp byline="true" file="jboss-5.1.0.GA/server/all-with-hornetq/deploy/hornetq-ra.rar/META-INF/ra.xml" match="%lt;resourceadapter-class%gt;org.hornetq.ra.HornetQResourceAdapter%lt;/resourceadapter-class%gt;" replace="%lt;resourceadapter-class%gt;org.hornetq.ra.HornetQResourceAdapter%lt;/resourceadapter-class%gt;\n      %lt;config-property%gt;\n        %lt;description%gt;The connection TTL%lt;/description%gt;\n        %lt;config-property-name%gt;ConnectionTTL%lt;/config-property-name%gt;\n        %lt;config-property-type%gt;java.lang.Long%lt;/config-property-type%gt;\n        %lt;config-property-value%gt;-1%lt;/config-property-value%gt;\n      %lt;/config-property%gt;
"  /^> >> build.xml
echo	^</target^> >> build.xml
