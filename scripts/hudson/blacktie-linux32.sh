...
ant dist -DBT_HOME=$WORKSPACE/trunk/dist/ -DVERSION=blacktie-3.0.0.M2 -DMACHINE_ADDR=`hostname` -DJBOSSAS_IP_ADDR=localhost -Dbpa=centos55x32
if [ "$?" != "0" ]; then
	ps -f
	$WORKSPACE/jboss-5.1.0.GA/bin/shutdown.sh -S && cd .
	killall -9 testsuite
...
# RUN ALL THE SAMPLES
cd $WORKSPACE/trunk/dist/blacktie-3.0.0.M2/
. setenv.sh
if [ "$?" != "0" ]; then
	ps -f
	$WORKSPACE/jboss-5.1.0.GA/bin/shutdown.sh -S && cd .
...
cp $WORKSPACE/trunk/dist/blacktie-3.0.0.M2/examples/xatmi/security/hornetq-*.properties $WORKSPACE/jboss-5.1.0.GA/server/all-with-hornetq/conf/props
