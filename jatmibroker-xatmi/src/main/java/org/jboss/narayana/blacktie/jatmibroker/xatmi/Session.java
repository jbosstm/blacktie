 ...
 * from the TPSVCINFO structure for a service (assuming the service was invoked within the scope of a tpconnect).
 * 
...
public class Session {
    ...
    /**
     * The last event received by the session, this is either discon, SUCC, FAIL, ERR
     */
     ...
     * 
     * @param connection The connection that created the session
     * @param transport The transport to create actors on.
     * @param cd The cd of the session
     * 
     * @throws ConnectionException In case the receiver cannot be created.
     * @see Connection#tpconnect(String, Buffer, int, int)
     */
    Session(Connection connection, String serviceName, Transport transport, int cd) throws ConnectionException {
     ...
     * @param replyTo The client to reply to.
     * @throws ConnectionException In case the receiver or sender cannot be established.
     * @see Connection#createServiceSession(String, int, Object)
     */
    Session(Connection connection, Transport transport, int cd, Object replyTo) throws ConnectionException {
    ...
    /**
     * Set the state of the session using the flags. This is so we can respond from the service easily with the <code>ACK</code>
     * that the client expects in initialization of the connection.
     * 
     * @param flags
     */
    ...
    void close() throws ConnectionException {
        ...
        if (closed) {
            throw new ConnectionException(Connection.TPEPROTO, "Session already closed");
        }
        if (sender != null) {
     ...
     * 
     * @param toSend The outbound data
     * @param len The length of the data
     * @param flags The flags to use
     * @throws ConnectionException If the message cannot be sent.
     */
    public int tpsend(Buffer toSend, int flags) throws ConnectionException {
        ...
        if (this.lastEvent > -1) {
            throw new ResponseException(Connection.TPEEVENT, "Event existed on descriptor: " + lastEvent, lastEvent, lastRCode,
                    null);
        } else if (!canSend) {
            throw new ConnectionException(Connection.TPEPROTO, "Session can't send");
        }
        ...
        if (sender != null) {
            ...
            throw new ConnectionException(Connection.TPEPROTO, "Session in receive mode");
        }
        return toReturn;
    }
     ...
     * @throws ConnectionException If the message cannot be received or the flags are incorrect
    ...
    public Buffer tprecv(int flags) throws ConnectionException, ConfigurationException {
        ...
        if (!canRecv) {
            throw new ConnectionException(Connection.TPEPROTO, "Session can't receive");
        }
        ...
        if ((m.flags & Connection.TPRECVONLY) == Connection.TPRECVONLY) {
            throw new ResponseException(Connection.TPEEVENT, "Reporting send only event", Connection.TPEV_SENDONLY, m.rcode,
                    received);
        } else if (m.rval == EventListener.DISCON_CODE) {
            close();
            throw new ResponseException(Connection.TPEEVENT, "Received a disconnect event", Connection.TPEV_DISCONIMM, m.rcode,
                    received);
        } else if (m.rval == Connection.TPSUCCESS || m.rval == Connection.TPFAIL) {
            log.debug("Completed session is being closed: " + cd);
            close();
            if (m.rval == Connection.TPSUCCESS) {
                throw new ResponseException(Connection.TPEEVENT, "Service completed successfully event",
                        Connection.TPEV_SVCSUCC, 0, received);
            } else if (m.rcode == Connection.TPESVCERR) {
                throw new ResponseException(Connection.TPEEVENT, "Service received an error", Connection.TPEV_SVCERR, m.rcode,
                        received);
            } else {
                throw new ResponseException(Connection.TPEEVENT, "Service received a fail", Connection.TPEV_SVCFAIL, m.rcode,
                        received);
            }
        }
        return received;
