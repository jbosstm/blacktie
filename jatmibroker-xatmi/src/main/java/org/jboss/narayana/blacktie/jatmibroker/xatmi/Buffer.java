...
public abstract class Buffer implements Serializable {
     ...
     * @see {@link X_COMMON}
     */
    Buffer(String type, String subtype, boolean requiresSerialization, List<Class> supportedTypes, Properties properties,
    ...
    private void format(String[] keys, Class[] types, int[] lengths, int[] counts) throws ConnectionException {
        ...
        if (keys.length != types.length || types.length != lengths.length) {
            throw new ConnectionException(Connection.TPEINVAL, "Invalid format, each array description should be same length");
        }
        this.keys = keys;
        this.types = types;
    ...
    public void deserialize(byte[] data) throws ConnectionException {
        ...
        if (requiresSerialization) {
            if (!deserialized && data != null) {
                if (keys == null) {
                    throw new ConnectionException(Connection.TPEITYPE, "Message format not provided");
                }
                ByteArrayInputStream bais = new ByteArrayInputStream(data);
                DataInputStream dis = new DataInputStream(bais);
                for (int i = 0; i < types.length; i++) {
                    if (!supportedTypes.contains(types[i])) {
                        throw new ConnectionException(Connection.TPEITYPE, "Cannot read type from buffer " + types[i]);
                    }
                    ...
                    try {
                        if (types[i] == int.class) {
                            ...
                            throw new ConnectionException(Connection.TPEITYPE, "Could not deserialize: " + types[i]);
                        }
                    } catch (IOException e) {
                        throw new ConnectionException(Connection.TPEITYPE, "Could not parse the value as: " + keys[i]
            ...
            for (int i = 0; i < types.length; i++) {
                ...
                try {
                    if (types[i] == int.class) {
                        ...
                        throw new ConnectionException(Connection.TPEOTYPE, "Could not serialize: " + types[i]);
                    }
                } catch (IOException e) {
                    throw new ConnectionException(Connection.TPEOTYPE, "Could not parse the value as: " + keys[i]
                            + " was not a " + types[i] + " and even if it was an array of that type its length was not: "
                            + lengths[i]);
                }
            }
    ...
    protected Object getAttributeValue(String key, Class type) throws ConnectionException {
        if (!formatted) {
            throw new ConnectionException(Connection.TPEPROTO, "Message not formatted");
        }
        int position = -1;
        for (int i = 0; i < keys.length; i++) {
        ...
        if (position == -1) {
            throw new ConnectionException(Connection.TPEITYPE, "Key is not part of the structure: " + key);
        } else if (types[position] != type) {
            throw new ConnectionException(Connection.TPEITYPE, "Key is not request type, it is a: " + types[position]);
