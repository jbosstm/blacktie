...
public class TPSVCINFO implements Serializable {
     ...
     */
    TPSVCINFO(String name, Buffer buffer, int flags, Session session, Connection connection, int len) {
        this.name = name;
        this.buffer = buffer;
        this.flags = flags;
    ...
    public Session getSession() throws ConnectionException {
        if (session == null) {
            throw new ConnectionException(Connection.TPEPROTO, "Not a TPCONV session");
        }
        return session;
    }
