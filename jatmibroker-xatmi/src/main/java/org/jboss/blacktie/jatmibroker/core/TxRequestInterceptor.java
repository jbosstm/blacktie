...
public class TxRequestInterceptor extends LocalObject
...
/*
	...
	{
		System.out.println("TxRequestInterceptor.receive_request_service_contexts");
		try {
			TaggedComponent comp = ri.get_effective_component(TxIORInterceptor.TAG_OTS_POLICY);
			ior_to_control(extract_ior_from_context(ri));
			// must be transactional
			// TxServerClientInterceptor
		} catch (org.omg.CORBA.SystemException e) {
			System.out.println("Not a transactionalal request: " + e.getMessage());
		} catch (Throwable e) {
			System.out.println("Unexpected error whilst checking for a transactional request: " + e);
		}
	}
*/
	public void send_request(ClientRequestInfo ri)
	{
		...
		if (curr != null) {
			...
			String ior = AtmiBrokerServerImpl.orb.object_to_string(control);
			System.out.println("tx ior: " + ior);
			ri.add_request_service_context(new ServiceContext(tx_context_id, ior.getBytes()), false);
		}
	}
	...
	public void receive_request_service_contexts(ServerRequestInfo ri)
	{
		...
		try {
			ServiceContext serviceContext = ri.get_request_service_context(tx_context_id);
			byte[] data = serviceContext.context_data;
			System.out.println("TxRequestInterceptor.receive_request_service_contexts: data: " + data);
		} catch (BAD_PARAM e) {
			System.out.println("TxRequestInterceptor.receive_request_service_contexts: not transactional");
			// not a transactional request - ignore
		} catch (SystemException e) {
			System.out.println("receive_request_service_contexts error: " + e.getMessage());
		}
	}
